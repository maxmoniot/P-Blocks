...
// Fonction pour nettoyer toutes les zones de dépôt (appelée uniquement après chargement HTML)
function cleanupAllDropZones() {
    // Supprimer TOUS les placeholders (cette fonction n'est appelée que quand c'est sûr)
    document.querySelectorAll('#program-drag-placeholder').forEach(p => {
        console.log('Placeholder program-drag supprimé');
        p.remove();
    });
    
    document.querySelectorAll('#menu-drag-placeholder').forEach(p => {
        console.log('Placeholder menu-drag supprimé');
        p.remove();
    });
    
    document.querySelectorAll('.drag-placeholder').forEach(p => {
        console.log('Placeholder générique supprimé');
        p.remove();
    });
    
    // Supprimer tout div avec border dashed qui est enfant direct de program-blocks
    document.querySelectorAll('#program-blocks > div').forEach(div => {
        const style = div.style.cssText || '';
        if (style.includes('dashed') && !div.classList.contains('program-block') && !div.classList.contains('block-capsule')) {
            console.log('Div avec border dashed supprimé');
            div.remove();
        }
    });
    
    // Supprimer les zones nested-blocks orphelines (pas dans un .program-block)
    const allNestedBlocks = document.querySelectorAll('.nested-blocks');
    allNestedBlocks.forEach(zone => {
        // Vérifier si cette zone est bien dans un block-capsule
        const parentCapsule = zone.closest('.block-capsule');
        
        if (!parentCapsule) {
            // Zone orpheline → supprimer complètement
            console.log('Zone nested-blocks orpheline supprimée');
            zone.remove();
            return;
        }
        
        // Retirer les classes de drag
        zone.classList.remove('drag-over');
        
        // Vérifier si la zone contient des blocs
        const hasBlocks = zone.querySelector('.program-block');
        
        if (hasBlocks) {
            // A des blocs → retirer empty
            zone.classList.remove('empty');
        } else {
            // Vide et dans une capsule valide → ajouter empty
...
function loadLevel() {
    ...
    if (currentMode === 'student' || window.isPreviewMode || window.isStudentLoadMode) {
        ...
        // Nettoyer les zones de dépôt UNIQUEMENT après le chargement du HTML sauvegardé
        setTimeout(() => {
            cleanupAllDropZones();
        }, 100);
    }
...
async function validateLevel() {
    ...
    resetTurtle();
    getActiveGrid();
    await executeBlocks(blocks);  // ← Juste ajouter await !
...
// Mode professeur
    ...
    window.cleanupAllDropZones = cleanupAllDropZones;
