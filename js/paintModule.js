...
function findRepeatPattern(program, startIndex) {
    ...
    // Tester différentes longueurs de motif (de 1 à 10 blocs)
    // IMPORTANT: Commencer par 1 pour détecter les répétitions simples (ex: 4x avancer)
    for (let patternLength = 1; patternLength <= Math.min(10, program.length - startIndex); patternLength++) {
        ...
        // Calculer un score : privilégier les motifs courts répétés souvent
        // Score = répétitions × (1 + bonus si motif court)
        // Un motif de longueur 1 répété 4 fois aura un meilleur score qu'un motif de longueur 2 répété 2 fois
        if (repeatCount >= 2) {
            const score = repeatCount * (patternLength === 1 ? 2 : 1);
