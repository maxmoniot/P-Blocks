...
function loadSavedBlock(blockData, container) {
    ...
    // IMPORTANT : Ajouter ondragover et ondrop pour que le bloc puisse RECEVOIR un drop
    ...
    blockElement.ondrop = function(e) {
        ...
        if (dragging && dragging !== blockElement) {
            ...
            if (e.clientY < midpoint) {
                container.insertBefore(dragging, blockElement);
            } else {
                container.insertBefore(dragging, blockElement.nextSibling);
            }
        }
        document.querySelectorAll('.drag-over-top, .drag-over-bottom').forEach(el => {
            el.classList.remove('drag-over-top', 'drag-over-bottom');
